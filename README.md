...
| 模式 | 能力 | 费用 |
|------|------|------|
| 离线本地 | Whisper-tiny 端侧语音识别 + Qwen2-VL-2B 端侧视觉理解 + FlutterTts 本地播报 | 0 |
...
# 可选：强制指定下载源
./download_models.sh hf    # 强制使用 HuggingFace
./download_models.sh ms   # 强制使用魔搭社区（ModelScope）
./download_models.sh auto  # 自动选择（默认）：Whisper 用 HF，Qwen-VL 用魔搭

# 环境变量（可选，用于私有模型或加速下载）
export HF_TOKEN=your_huggingface_token   # HuggingFace Access Token
export MS_TOKEN=your_modelscope_token     # 魔搭社区 Access Token
```

> **提示**：国内用户推荐使用 `./download_models.sh ms` 从魔搭社区下载，速度更快。Whisper TFLite 格式在魔搭无官方镜像，始终从 HuggingFace 下载。
