...
-   **千问**
    ...
    -   千问Max
        ...
        -   Qwen3.7-Max：qwen3.7-max、qwen3.7-max-preview、qwen3.7-max-2026-05-17 及之后的快照版本（仅[Responses API](#7b82860922v0y)支持）
    ...
    -   千问Plus
        ...
        -   Qwen3.7-Plus：qwen3.7-plus、qwen3.7-plus-2026-05-26及之后的快照版本（仅[Responses API](#7b82860922v0y)支持）
            
        -   Qwen3.6-Plus：qwen3.6-plus、qwen3.6-plus-2026-04-02及之后的快照版本（仅[Responses API](#7b82860922v0y)支持）
    ...
    -   千问Flash
        ...
        -   Qwen3.6-Flash：qwen3.6-flash、qwen3.6-flash-2026-04-16及之后的快照版本（仅[Responses API](#7b82860922v0y)支持）
...
-   千问Max
    ...
    -   Qwen3.7-Max：qwen3.7-max、qwen3.7-max-2026-05-20及之后的快照版本（仅[Responses API](#7b82860922v0y)支持）
        
-   千问Plus
    
    -   qwen3.7-plus、qwen3.7-plus-2026-05-26及之后的快照版本（仅[Responses API](#7b82860922v0y)支持）
        
    -   qwen3.6-plus、qwen3.6-plus-2026-04-02及之后的快照版本（仅[Responses API](#7b82860922v0y)支持）
...
-   `agent`：可多次调用联网搜索工具与大模型，实现多轮信息检索与内容整合。
    ...
    > 启用该策略时，每次调用额外收费，参见 [计费说明](#92ce83df3a599) 。
...
> 关于Responses API的使用说明、代码示例和迁移指南，请参见 [OpenAI兼容-Responses](https://help.aliyun.com/zh/model-studio/compatibility-with-openai-responses-api) 。
...
A：不会。系统不会将原始问题直接透传给搜索引擎，而是对查询意图进行深度解析和多层脱敏处理，仅提取可用于检索的关键信息片段。从原始 query 到实际检索会经过多次改写，确保敏感内容不会明文外泄。阿里云采用数据加密和隐私保护措施确保安全，具体请参考[服务协议](https://terms.alibabacloud.com/legal-agreement/20230516171057961/20231221152498439.html)。
